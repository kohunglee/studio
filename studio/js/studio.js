 ...
 */
    ...
    /**
     * 刷新文件列表显示（为了更新文件列表数据，同时也保留原有的状态）
     */
	...
	function st_openProject(url){
		...
		function(data){
            ...
            let list_json = JSON.parse(data),
                htmlC     = '<a href="javascript:void(0);" onclick="st_showProj()" class="file_backProject">返回项目列表 <i class="icofont-logout"></i></a><ul class="scroll_ul" link="' + url + '" id="file_ul">';
...
$(document).ready(function(){
    ...
    $("#addfile-button").click(function () {
        ...
        {
            url: addFileSourceUrl + "/" + newFileName
        },
        function(data){
            if(data == "add_file_success"){
                $("#addFile").modal('hide');  // 关闭模态窗

                // 如果非插件根目录，模拟重复按两遍展开关闭文件夹以实现刷新，否则刷新插件根目录
                if($ele.length > 0){
                    st_openFile($ele);
                    st_openFile($ele);
                } else {
